...
from adminmainscr import *
from admin_ins import *
from adminforgot import *
import pymysql
def adminlogin():
    t=tkinter.Tk()
    t.geometry('700x700')
    ...
    def logincheck():
        ...
        cur=db.cursor()
        sql="select count(*) from admin where aname='%s' and apass='%s'"%(x,y)
        cur.execute(sql)
        data=cur.fetchone()
        w=int(data[0])
        if w==0:
    ...
    h.place(x=160,y=400)
    bt=Button(c,text='Create account',font=5,bg='green',fg='white',command=admininsert)
    bt.place(x=310,y=400,height=20,width=150)
    bt1=Button(c,text='forgot id & pass',fg='black',bg='yellow',command=adminforgot)
