...
def feedbackupdate():
    ...
    def matchdata():
        ...
        if w==0:
            ...
            db=pymysql.connect(host='localhost',user='root',password='youtube',database='testdb')
            cur=db.cursor()
            sql="update feedback set teacherid='%s',majorsub='%s',rating='%s',remark='%s' where studid='%s'"%(y,z,p,r,x)
            cur.execute(sql)
            db.commit()
    ...
    s=Label(t,text='FEEDBACK UPDATE',font=40,fg='black',bg='red')
    s.place(x=275,y=50)
