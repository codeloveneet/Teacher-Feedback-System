...
def collegeinsert():
    ...
    def savedata():
        ...
        if(len(x)==0 or len(y)==0 or len(z)==0 or len(p)==0 or len(q)==0 or len(r)==0):
            ...
            cur=db.cursor()
            sql="insert into college values('%s','%s','%s','%s','%s','%s')"%(x,y,z,p,q,r)
            cur.execute(sql)
            db.commit()
    ...
    def checkdata():
        ...
        db=pymysql.connect(host='localhost',user='root',password='youtube',database='testdb')
        cur=db.cursor()
        sql="select count(*) from college where collegeid='%s'"%(x)
        cur.execute(sql)
        data=cur.fetchone()
        if int(data[0])==0:
    ...
    s=Label(t,text='COLLEGE INSERT',font=20,fg='black',bg='red')
    s.place(x=250,y=50)
    ...
    bt1.place(x=250,y=450,height=40,width=60)
    bt2=Button(t,text='CLEAR',bg='orange',command=clear)
    bt2.place(x=350,y=450,height=40,width=60)
    t.mainloop()