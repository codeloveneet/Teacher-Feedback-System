...
def userupdate():
    ...
    def updatedata():
        ...
        db=pymysql.connect(host='localhost',user='root',password='youtube',database='testdb')
        cur=db.cursor()
        sql="update users set uname='%s',upass='%s',uemail='%s',uaddress='%s',ucity='%s',uphone='%s' where userid='%s'"%(y,z,p,q,r,u,x)
        cur.execute(sql)
        db.commit()
    ...
    def clear():
        ...
        f1.delete(0,100)
        g1.delete(0,100)
    s=Label(t,text='USER UPDATE',font=40,fg='black',bg='red')
    s.place(x=200,y=50)
    ...
    bt.place(x=150,y=480,height=40,width=80)
    bt1=Button(t,text='UPDATE',bg='orange',command=updatedata)
    bt1.place(x=250,y=480,height=40,width=80)
    bt2=Button(t,text='CLEAR',bg='orange',command=clear)
    bt2.place(x=350,y=480,height=40,width=80)
    t.mainloop()
